            ...
            if (UserId != null)
            {
                if (ecMSR.Entities.Count > 0)
                {
                    ...
                    for (int i = 0; i < ecMSR.Entities.Count; i++)
                    {
                        ...
                        {
                            if (((OptionSetValue)ecMSR.Entities[i].Attributes["imc_servicerequeststatus"]).Value == 1)
                                strStatus = "Pending";
                            else if (((OptionSetValue)ecMSR.Entities[i].Attributes["imc_servicerequeststatus"]).Value == 2)
                                strStatus = "Rejected";
                            else if (((OptionSetValue)ecMSR.Entities[i].Attributes["imc_servicerequeststatus"]).Value == 3)
                                strStatus = "Approved";
                            else if (((OptionSetValue)ecMSR.Entities[i].Attributes["imc_servicerequeststatus"]).Value == 4)
                                strStatus = "In Progress";
                            else if (((OptionSetValue)ecMSR.Entities[i].Attributes["imc_servicerequeststatus"]).Value == 5)
                                strStatus = "Completed";
                        }
